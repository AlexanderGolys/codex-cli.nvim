...
function M.expand_token(token, context)
    ...
    if token == "&selection" and context.selection_text then
        ...
        if start_row == end_row then
            return ("Line %d selected in @%s: %s"):format(start_row, context.relative_path,
...
function M.quick_prompts(context)
    local prompts = {
        ...
        },
        {
            ...
            text =
            "Fix the current buffer diagnostics, or explain clearly why any remaining ones should be ignored.\n\n&buff_diagnostics",
        },
        {
            id = "fix-all-diagnostics",
            label = context and context.project_root and "Fix all diagnostics" or "Fix all diagnostics (disabled)",
            text =
            "Fix the project diagnostics in a sensible order, grouping related issues together and noting any follow-up validation.\n\n&all_diagnostics",
    ...
    if context and context.selection_text then
        prompts[#prompts + 1] = {
            ...
            text =
            "Explain the selected code in detail, including how it is used and any edge cases around it.\n\n&selection",
        }
    end

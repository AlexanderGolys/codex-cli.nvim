...
- Optional project-local file generated by Clodex and passed to Codex CLI as `-c model_instructions_file=...`.
- Default is empty, so manual project conversations do not get any generated extra instructions unless you opt in.
- Relative paths are resolved per project. A setting like `.clodex/manual-history.md` becomes `<project>/.clodex/manual-history.md`.
- When enabled, project sessions and direct `codex exec` runs tell Codex how to append or update `queues.history` records in that project's workspace JSON after hand-written implementation or bug-fix prompts.
- This keeps manual Codex conversations compatible with the queue history view without modifying the repository's `AGENTS.md`.

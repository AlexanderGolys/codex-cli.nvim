...
6. Find the queue item with the provided id in `.clodex/queued.json`, `.clodex/implemented.json`, or `.clodex/history.json`.
7. If it is still in `.clodex/queued.json`, remove it from there and add the same item to the front of `.clodex/implemented.json` without changing its `id`.
8. If it is already in `.clodex/implemented.json`, update it in place.
9. If it is already in `.clodex/history.json`, update it in place instead of duplicating it.
10. Set `history_summary`, `history_commits` (array of commit ids) when commits exist, `history_completed_at`, and refresh `updated_at`.
11. Preserve `execution_instructions` on queued items. Do not show or rewrite that hidden field unless the task explicitly requires it.
12. After updating the current item, inspect `.clodex/queued.json`. If another queued item remains, continue immediately with the next queued item and repeat this workflow.
13. Each next queued item already includes the same hidden execution instructions, so rely on the current prompt's instructions again instead of trying to remember the whole loop from earlier in the session.
14. Repeat until `.clodex/queued.json` is empty. Do not start prompts that are only in `.clodex/planned.json` or `.clodex/implemented.json`.
...
7. Set `kind` to `bug` for bug fixes or regressions, otherwise use the closest existing queue category such as `todo`, `refactor`, `adjustment`, `idea`, or `ask`.
